...
role: (existingUser && existingUser.role) || defaultUser.role,
...
const input = event && event.target ? event.target : document.getElementById("cloudinaryPhotoInput");
if(input){
input.value = "";
}
...
showPhotoUploadMessage(slot, "Cloudinary a refuse l'envoi de la photo.", true);
}
}
...
const photoSource = user.photoUrl ? "Photo partagee Cloudinary active." : "Photo locale active : <strong>photos/" + user.username + ".png</strong>.";
